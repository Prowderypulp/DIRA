//! Per-region worker: opens BAM/VCF/FASTA handles once, processes all indels
//! in the region. This is the key win over Python — handles are reused across
//! many sites within a region.
...
pub fn process_region(
    ...
    while let Some(r) = vcf.read(&mut record) {
        ...
        let ref_allele = std::str::from_utf8(alleles[0])?.to_string();
        let alt_allele = std::str::from_utf8(alleles[1])?.to_string();
        ...
        let pos0: i64 = pos - 1;
        let indel_length = alt_allele.len() as i64 - ref_allele.len() as i64;
        let is_insertion = indel_length > 0;
        let indel_length_abs = indel_length.abs();

        if let Some(row) = process_site(
            &mut bam,
            &fa,
            &region.chrom,
            pos,
...
fn process_site(
    ...
    let fetch_start = std::cmp::max(0, pos0 - 1);
    let fetch_end = pos0 + 2;
    bam.fetch((chrom, fetch_start, fetch_end))?;

    // Collect overlapping reads + cache aligned pairs.
    let mut all_records: Vec<Record> = Vec::new();
    let mut all_pairs: Vec<AlignedPairs> = Vec::new();
    ...
    while let Some(r) = bam.read(&mut rec) {
        ...
        // pysam: read.reference_start > pos0 OR read.reference_end <= pos0 -> skip
        // rust-htslib: cigar.end_pos() gives reference end (1-past-last).
        let cigar = rec.cigar();
        let read_end = cigar.end_pos();
        if read_start > pos0 || read_end <= pos0 {
            continue;
        }

        let pairs = AlignedPairs::from_record(&rec);
        let supports = read_supports_indel(
            &cigar, read_start, pos0, is_insertion, indel_length_abs,
        );

        let idx = all_records.len();
        if supports {
            alt_indices.insert(idx);
            alt_qnames.insert(rec.qname().to_vec());
        }
        all_pairs.push(pairs);
        all_records.push(rec.clone());
    }

    let depth = all_records.len();
    if depth == 0 {
        return Ok(None);
    }
    ...
    // ── Pre-fetch flanking ref sequence (single fetch per site) ──────────
    let flank_lo = std::cmp::max(0, pos0 - 5) as usize;
    let flank_hi = (pos0 + 5) as usize;
    ...
    // ── Reference context window for homopolymer + GC + tandem repeat ────
    let ctx_start = std::cmp::max(0, pos0 - 50) as usize;
    let ctx_end = (pos0 + 50 - 1) as usize;
    let ctx_seq: Vec<u8> = fa
        .fetch_seq(chrom, ctx_start, ctx_end)
        .map(|s| s.to_ascii_uppercase())
        .unwrap_or_default();
    let (gc, homopoly, tr_unit, tr_length) = compute_context(&ctx_seq, pos0 - ctx_start as i64);

    // ── Per-read scalar stats ────────────────────────────────────────────
    ...
    for (i, r) in all_records.iter().enumerate() {
        ...
        // NBQ + read-end distance (uses cached pairs)
        let pairs = &all_pairs[i];
        let qpos_at_site = pairs.qpos_at(pos0);
        let nbq = compute_nbq(quals, &pairs.pairs, pos0, 5);
        ...
        // Flanking BQ (uses cached pairs + pre-fetched ref seq)
        let (fbq, mm, al) = flanking_bq(quals, &r.seq().as_bytes(), &pairs.pairs,
                                        pos0, &flank_seq, flank_offset, 5);
        ...
        if is_alt {
            ...
            flank_mm_total += mm;
            flank_al_total += al;
            alt_sigs.push(cigar_signature(&r.cigar(), r.pos(), pos0, 3));
            // Fragment support: mate also in alt set?
    ...
    // Insertion sequence entropy
    let insert_entropy = if is_insertion {
        entropy_bytes(alt_allele.as_bytes())
    } else { 0.0 };

    // CIGAR agreement + haplotype entropy
    ...
    Ok(Some(FeatureRow {
        chrom: chrom.to_string(), pos, ref_allele, alt_allele,
        ...
        bq_drop_alt, indel_length, is_insertion: is_insertion as i64, insert_entropy,
